...
export { broadcast, gamerule, getOnlinePlayers, hasPermission, showChatGamesMenu, showMainMenu, showMemberMenu }

function hasPermission(source, role) {
  const isAuthorized = (role === "admin" && source.hasTag("admin")) || (role === "member")
...
async function showPlayerMenu(source) {
  try {
    ...
    const actions = [() => showChangelog(source), () => showTeleportMenu(source), () => showBanManagementMenu(source), () => showMutePlayerMenu(source), () => showKickPlayerMenu(source), () => showItemAndBlockMenu(source), () => ViewInventory(source), () => showMoneyMenu(source), () => showMainMenu(source)]
...
async function showItemAndBlockMenu(source) {
  try {
    ...
    const betterGUI = new ModalFormData()
      ...
      .textField("item/block id (e.g., minecraft:grass)", "enter item/block id", {
        defaultValue: "",
    ...
    if (targetPlayerName === "@a") {
      ...
      const targetPlayer = players.find(player => player.name === targetPlayerName)
      if (targetPlayer) {
        source.runCommand(`give ${targetPlayerName} ${itemID} ${amount}`)
      } else {
        source.runCommand(`tellraw @s {"rawtext":[{"text":"§cError: Player '${targetPlayerName}' not found."}]}`)
      }
    }
...
async function showMutePlayerMenu(source) {
  try {
    ...
    if (players.length === 0) {
      source.runCommand(`tellraw @s {"rawtext":[{"text":"§cNo players online to mute/unmute."}]}`)
    ...
    const selectedPlayer = world.getPlayers().find(player => player.name === selectedPlayerName)

    if (selectedPlayer) {
      const isMuted = action === "muted"
      selectedPlayer.setDynamicProperty("isMuted", isMuted)
      const message = isMuted ? "§eWarning: You have been muted." : "§aYou have been unmuted."
      const soundCategory = isMuted ? "warning" : "success"

      await selectedPlayer.runCommandAsync(`tellraw @s {"rawtext":[{"text":"${message}"}]}`)
      playSound(selectedPlayer, soundCategory)

      updatePlayerNameTag(selectedPlayer)
      source.runCommandAsync(`tellraw @s {"rawtext":[{"text":"Player ${selectedPlayerName} has been ${action}."}]}`)
      playSound(source, "action")
    } else {
      source.runCommandAsync(`tellraw @s {"rawtext":[{"text":"Player ${selectedPlayerName} is not online."}]}`)
      playSound(source, "error")
    }
  } catch (error) {
...
function ViewInventory(source) {
  ...
  playSound(source, "success")
  source.runCommand(`tellraw @s {"rawtext":[{"text":"§aInventory viewer added to your items."}]}`)
}
...
async function showTeleportMenu(source) {
  try {
    const players = world.getPlayers().map(player => player.name)
    if (players.length === 0) {
      source.runCommand(`tellraw @s {"rawtext":[{"text":"${Lang.t(source, "tp.err.no_player")}"}]}`)
      playSound(source, "error")
      return
    }
    ...
    const teleportForm = new ModalFormData()
      ...
      .dropdown(Lang.t(source, "tp.select.mode"), [
        Lang.t(source, "tp.mode.to"),
        Lang.t(source, "tp.mode.here")
      ], {
        defaultValueIndex: 0,
      })
...
async function showMainMenu(source) {
  if (!source.hasTag("admin")) {
    ...
    source.sendMessage([`§8§l[§r§c§laccess denied§r§8§l]§r`, `§7this menu requires administrative privileges.`, ``, `§frequired: §cadmin tag`, `§7contact a server administrator for access.`].join("\n"))
    return
  }
