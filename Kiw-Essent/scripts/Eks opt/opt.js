...
// Variable untuk melacak apakah konfigurasi sudah dicoba dimuat
let configLoaded = false;

// Configuration management
function getConfig() {
  try {
    // Jika konfigurasi sudah pernah dimuat, gunakan nilai dari memori
    if (configLoaded) {
    ...
    system.run(() => {
      try {
        ...
        // Parsing dan update konfigurasi lokal
...
function saveConfig(config) {
  try {
    // Selalu update konfigurasi lokal terlebih dahulu
    ...
    system.run(() => {
      try {
        ...
        // Kita tetap menggunakan konfigurasi yang sudah diupdate di memori
      }
    });
  } catch (error) {
...
function initializeConfig() {
  try {
    ...
    // Tetap gunakan konfigurasi default jika gagal memuat
  }
}
...
world.afterEvents.playerBreakBlock.subscribe(
  ({ block, brokenBlockPermutation, dimension, player }) => {
    ...
    try {
      ...
      config = antiNukerConfig; // Gunakan konfigurasi default jika gagal
    }
...
function setTickTimeout(callback, tick, loop = false) {
  try {
    ...
    // Tetap coba jalankan callback segera jika terjadi error
    try {
...
async function toggleAntiNuker(player, feature) {
  try {
    ...
    try {
      ...
      // Tetap lanjutkan karena konfigurasi lokal sudah diperbarui
    }

    // Tampilkan pesan ke player
    player.runCommand(
...
function createToggleForm() {
  try {
    ...
    // Buat form dasar jika terjadi error
