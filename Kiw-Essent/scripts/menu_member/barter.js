...
system.runTimeout(() => {
  try {
    ...
    // Error init barter storage, silent
  }
}, 20);
function saveBarterSessions() {
  try {
    ...
    // Error saving barter sessions, silent
  }
}
  ...
  try {
    ...
    for (const data of barterData) {
      try {
        ...
        // Error reconstructing barter session, silent
      }
    }
  } catch (error) {
    // Error loading barter sessions, silent
  }
}
...
world.afterEvents.playerJoin.subscribe((event) => {
  try {
    ...
    // Error reloading barter sessions on player join, silent
  }
});
class BarterSession {
