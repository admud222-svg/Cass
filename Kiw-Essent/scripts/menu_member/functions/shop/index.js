...
function showTransactionMenu(
  ...
  let brick = new ModalFormData()
    .title(`§6SHOP UI §t§p§a`)
    .textField(
      `§7My ${currencyName} : §g${currencySymbol}${formatShopBalance(moneyAmount)}\n§7Item : §f${item.name}\n§7Buy x1 : ${costDisplay}\n§7Sell x1 : §g${item.sell}\n\n§7Amount`,
      `Amount you want to buy or sell`,
      {
        defaultValue: "",
        tooltip: "Enter the amount to buy/sell",
      },
    )
    .toggle("§fSell / Buy", {
      defaultValue: true,
      tooltip: "ON = Buy / OFF = Sell",
    });
  brick.show(player).then((res) => {
    ...
    if (res.formValues[1] == true) {
      // Check for enchanted books - need enough empty slots
                  ...
                  try {
                    ...
                    if (inventory && inventory.container) {
                      ...
                      for (let i = 0; i < quantity; i++) {
                        try {
                          // Create enchanted book using ItemStack API
                          ...
                          // Add enchanted book to inventory (auto-finds empty slot)
                          const remainder = inventory.container.addItem(enchantedBook);
                          if (remainder) {
                            // Inventory full, spawn item on ground
        ...
        queueTransaction(
          ...
          async () => {
            try {
              ...
              // Get the item ID without minecraft: prefix for flexible matching
              ...
              for (let i = 0; i < container.size; i++) {
                ...
                if (slotItem) {
                  ...
                  // Check if the item matches - flexible matching for blocks with different IDs
