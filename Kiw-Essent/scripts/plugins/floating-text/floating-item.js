    ...
    try {
        ...
        for (const dimId of DIMENSIONS) {
            ...
            for (const item of items) {
                let comp;
                try {
                    // Try-catch block to handle potential invalid entities or API version differences
        ...
        // Group clusters that are physically close, regardless of type
        const usedIndices = new Set();
        const groups = [];

        // Sort by height first to prioritize vertical stacking order
        ...
        for (let i = 0; i < clustersList.length; i++) {
            ...
            for (let j = i + 1; j < clustersList.length; j++) {
                ...
                // If in same dimension and close horizontally
                if (a.dim.id === b.dim.id) {
                    const dist = Math.hypot(a.basePos.x - b.basePos.x, a.basePos.z - b.basePos.z);
                    // Vertical check: if they are somewhat aligned vertically
        ...
        for (const group of groups) {
            // Sort: Oldest first (bottom), Newest last (top). If same time, use key for stability.
            group.sort((a, b) => {
            ...
            for (let k = 0; k < group.length; k++) {
                group[k].pos.y += k * 0.35; // 0.35 block offset per item in stack
            }
        }
...
system.runInterval(() => { currentTick++; }, 5); // Optimized from 1
system.runInterval(() => { system.runJob(processItemsGenerator()); }, 10); // Optimized from 2
system.runInterval(() => { if (currentTheme === 'rainbow') updateRainbowItems(); }, 20); // Optimized from 10
system.runInterval(() => { updateRadar(); }, 20); // Optimized from 5
system.runInterval(cleanupOrphanedTexts, 200); // Optimized from 100
