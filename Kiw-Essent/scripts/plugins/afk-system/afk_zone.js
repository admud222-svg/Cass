...
// Import Money System if available. Handle gracefully if not.
let addMoney = null;
try {
...
// Initialize Database for AFK Zone
...
// Map<PlayerID, { zoneId: string, time: number }>
...
function getConfig() {
    ...
    // Migration: Convert old single-zone config to multi-zone
...
system.runInterval(() => {
    ...
    for (const player of world.getPlayers()) {
        ...
        // Find which zone the player is in (prioritize first found for now)
        ...
        if (activeZone) {
            // If player was in a different zone or no zone, reset timer
            let time = 0;
...
function createNewZone(player) {
    ...
    modal.show(player).then(res => {
        ...
        const newZone = {
            ...
            id: Date.now().toString(),
            name,
            rewards: { ...DEFAULT_ZONE.rewards } // Clone rewards
        };
