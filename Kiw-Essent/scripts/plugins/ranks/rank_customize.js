...
export function showRankCustomizeMenu(player) {
  ...
  // Combine custom ranks (first) then default ranks
...
function showRankEditMenu(player, rankId) {
  ...
  // Only show delete button for custom ranks
...
function showDeleteConfirmMenu(player, rankId) {
  ...
  new ActionFormData()
    ...
    .then(response => {
      ...
      // Fallback: If not found in simpleRanks (ghost rank), extract name from rankId
      if (!realName) {
