...
import { showServerMenu } from "./admin_menu/transfer-server/menu.js"
...
const MENU_ITEMS = [
  ...
  { key: "member.menu.btn.transfer_server", icon: "textures/ui/world_glyph_color", feature: "transferServer", handler: showServerMenu },
...
export function showMemberMenu(source, viewMode) {
  ...
  if (viewMode === "list") {
    form.button("§a§l" + Lang.t(source, "member.menu.btn.grid_mode"))
  } else {
    form.button("§a§l" + Lang.t(source, "member.menu.btn.list_mode"))
  }
  ...
  form.show(source).then(result => {
    ...
    if (result.selection === totalItems + 1) return
